...
(defvar ack-mode-map
  (let ((keymap (make-sparse-keymap)))
    ...
    (define-key keymap "n" 'next-error-no-select)
    (define-key keymap "p" 'previous-error-no-select)
    ;; (define-key keymap "\M-n" 'ack-next-file)
    ;; (define-key keymap "\M-p" 'ack-previous-file)
    (define-key keymap "\M-n" 'compilation-previous-file)
    (define-key keymap "\M-p" 'compilation-next-file)
