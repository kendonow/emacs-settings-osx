...
(defvar xc-use-comment-dwim-p nil 
  "whether to use emacs's builtin comment-dwim 〔newcomment.el〕 commands.")

(setq xc-use-comment-dwim-p t )

(defvar xc-line-comment-syntax-table nil 
  "a alist that maps major mode name to the lang's line comment beginning chars syntax.")
...
(defvar xc-line-comment-marker nil 
  "Current line comment string. e.g. # // ;")
      ...
      (progn 
        (if (xc-whole-line-is-comment-p) 
          ...
          (progn 
            (let () 
              (if (equal (point) 
                         (line-end-position)) ; if cursor is at end of line, comment at the end.
                  (progn 
                    (xc-comment-line "end")) 
                (progn 
                  (xc-comment-line))))))))))
...
(defun  comment-region-extend(&optional arg) 
  (interactive "p") 
  (princ arg)
  (if mark-active 
      (progn 
        (call-interactively 'xc-comment-region) 
        ;; (message "mark-active t") 
        (deactivate-mark))
...
(defun  uncomment-region-extend(&optional arg) 
  (interactive "p") 
  (if mark-active 
      (progn 
        (call-interactively 'xc-uncomment-region) 
        (message "mark-active t") 
        (deactivate-mark))
    (if (not arg ) 
        (xc-uncomment-line) 
...
;; (defun  fg-comment-paragraph(&optional arg) 
;;   (interactive "p") 
;;   (mark-paragraph arg)
;;   (call-interactively 'xc-comment-region) 
;; )
...
(defun  fg-uncomment-paragraph(&optional arg) 
  (interactive "p") 
  (let ((pos (point)))
    (mark-paragraph arg)
    ;; (if mark-active 
...
(global-set-key (kbd "C-c ;"    ) 'comment-region-extend)
(global-set-key (kbd "C-c '"    ) 'uncomment-region-extend)

(global-set-key (kbd "C-c C-;"    ) 'comment-region-extend)
(global-set-key (kbd "C-c C-'"    ) 'uncomment-region-extend)

;; (global-set-key (kbd "C-c ;"    ) 'fg-comment-paragraph)
;; (global-set-key (kbd "C-c '"    ) 'fg-uncomment-paragraph)


(global-set-key (kbd "C-c h"    ) 'fg-comment-paragraph)
(global-set-key (kbd "C-c H"    ) 'fg-uncomment-paragraph)
