...
(defstruct aj-visual-area buffer 
           window
           frame
           recover-buffer)
...
(defun aj-queue-push (item q) 
  ...
  (let ( (head (aj-queue-head q) ) 
         ...
         (c (list item) ) ) 
    (cond ((null (aj-queue-head q)) 
           (setf (aj-queue-head q) c) 
           (setf (aj-queue-tail q) c)) 
          (t (setf (cdr (aj-queue-tail q)) c) 
             (setf (aj-queue-tail q) c)))))
...
(defun aj-queue-pop (q) 
  ...
  (if (null (aj-queue-head q)) 
      (error 
       "[AceJump] Interal Error: Empty queue"))
...
(or (assq 'ace-jump-mode minor-mode-alist) 
    (nconc minor-mode-alist (list '(ace-jump-mode ace-jump-mode))))
...
(defvar ace-jump-mode-submode-list '(ace-jump-word-mode ace-jump-char-mode ace-jump-line-mode) 
...
;;; define the face
(defface ace-jump-face-background '((t (:foreground "gray40"))) 
...
The returned value is a list of `aj-position' record." 
  (loop for va in visual-area-list append 
        (let* ((current-window (aj-visual-area-window va)) 
          ...
          (with-selected-window current-window 
            (save-excursion 
              ...
              (let ((case-fold-search ace-jump-mode-case-fold)) 
                (loop while (re-search-forward re-query-string nil t) until (or (> (point)
                                                                                   end-point) 
                                                                                (eobp)) if (or
  ...
  (let ((left-leaf-node (- total-leaf-node 1)) 
    ...
    (while (> left-leaf-node 0) 
      ...
      (if (<= left-leaf-node max-child-node)
          ...
          (progn 
            (setf (cdr node) 
                  (loop for i from 1 to left-leaf-node collect (cons 'leaf nil)))
        ...
        (progn
          ...
          (setf (cdr node) 
                (loop for i from 1 to max-child-node collect 
                      (let ((n (cons 'leaf nil))) 
                        (aj-queue-push n q)
                        n))) 
    ...
    (while (not (null s))
      ...
      (let ((node (car s)))
        ...
        (cond ((eq (car node) 'branch)
              ...
              (t (message "[AceJump] Internal Error: invalid tree node type")))))))
...
(defun ace-jump-populate-overlay-to-search-tree (tree candidate-list) 
  ...
  (lexical-let* 
      ( ;; create the locally dynamic variable for the following function
...
(defun ace-jump-update-overlay-in-search-tree (tree keys) 
  ...
  (lexical-let* 
      ( ;; create dynamic variable for following function
       (key ?\0)
       ...
       (func-update-overlay 
        (lambda (node) 
          (let ((ol (cdr node))) 
            (overlay-put ol 'display (concat (make-string 1 key) 
                                             (let* ((pos (overlay-get ol 'aj-data)) 
                                               ...
                                               (cond
                                                ...
                                                ((string-equal subs "\n") "\n") 
                                                (t ""))))))))) 
    (loop for k in keys for n in (cdr tree) do 
          (progn
            ;; update "key" variable so that the function can use
            ;; the correct context
            (setq key k) 
            (if (eq (car n) 'branch) 
                (ace-jump-tree-preorder-traverse n func-update-overlay) 
...
(defun ace-jump-list-visual-area() 
  ...
  (cond ((eq ace-jump-mode-scope 'global) 
         (loop for f in (frame-list) append 
               (loop for w in (window-list f) collect (make-aj-visual-area :buffer (window-buffer w) 
                                                                           :window w 
                                                                           :frame f)))) 
        ((eq ace-jump-mode-scope 'frame) 
         (loop for w in (window-list (selected-frame)) collect (make-aj-visual-area :buffer
                                                                                    (window-buffer
                                                                                     w) 
                                                                                    :window w 
                                                                                    :frame
        ...
        ((eq ace-jump-mode-scope 'window) 
         (list (make-aj-visual-area :buffer (current-buffer) 
                                    :window (selected-window) 
                                    :frame  (selected-frame)))) 
        (t 
         (error 
          "[AceJump] Invalid ace-jump-mode-scope, please check your configuration"))))
...
relevant window." 
  (loop for va in visual-area-list
        ...
        if 
        (loop for vai in visual-area-list
        ...
        (with-selected-window (aj-visual-area-window va)
          ...
          (let ((ws (window-start))) 
            (set-window-buffer (aj-visual-area-window va) 
                               (aj-visual-area-buffer va)) 
            (set-window-start (aj-visual-area-window va) ws))
          va)))
...
"
  ...
  (if (or (null ace-jump-mode-move-keys) 
      ...
      (error 
       "[AceJump] Invalid move keys: check ace-jump-mode-move-keys"))
  ...
  (let* ((visual-area-list (ace-jump-list-visual-area)) 
    ...
    (cond
     ...
     (t
      ...
      (setq ace-jump-recover-visual-area-list (ace-jump-mode-make-indirect-buffer visual-area-list))
      ;; create background for each visual area
      (if ace-jump-mode-gray-background 
          (setq ace-jump-background-overlay-list 
                (loop for va in visual-area-list collect 
                      (let* ((w (aj-visual-area-window va)) 
                             (b (aj-visual-area-buffer va)) 
      ...
      (setq ace-jump-search-tree (ace-jump-tree-breadth-first-construct (length candidate-list) 
                                                                        (length
                                                                         ace-jump-mode-move-keys))) 
      (ace-jump-populate-overlay-to-search-tree ace-jump-search-tree candidate-list) 
      (ace-jump-update-overlay-in-search-tree ace-jump-search-tree ace-jump-mode-move-keys)

      ;; do minor mode configuration
      (cond ((eq ace-jump-current-mode 'ace-jump-char-mode) 
      ...
      (setq overriding-local-map 
            (let ( (map (make-keymap)) ) 
              ...
              ;; (define-key map (kbd "C-w") 'ace-jump-quick-exchange)
      ...
      (let ((p (car ace-jump-mode-mark-ring)))
        ...
        (if (eq (current-buffer) 
            ...
            (if (equal (aj-position-offset p) 
              ...
              (lexical-let 
                  ((po (aj-position-offset p))) 
                (setq mark-ring (ace-jump-move-first-to-end-if mark-ring 
                                                               (lambda (x) 
          ...
          (lexical-let 
              ((pb (aj-position-buffer p))) 
            (setq global-mark-ring (ace-jump-move-first-to-end-if global-mark-ring 
                                                                  (lambda (x) 
...
word-mode and char-mode" 
  ...
  (cond ((eq ace-jump-current-mode 'ace-jump-char-mode) 
        ...
        ((eq ace-jump-current-mode 'ace-jump-line-mode) nil) 
        (t nil)))
...
(defun ace-jump-char-mode (query-char) 
  "AceJump char mode" 
  (interactive (list (read-char "(Jump Char )Query Char:")))
...
" 
  ...
  (let ((index (/ prefix 4)) 
    ...
    (if (< index 0) 
        (error 
         "[AceJump] Invalid prefix command")) 
...
(defun ace-jump-move () 
  ...
  (interactive) 
  (let* ((index 
          (let ((ret (position (aref (this-command-keys) 0) ace-jump-mode-move-keys))) 
            (if ret 
                ret
    ...
    (cond
     ...
     (t (ace-jump-done) 
        (error 
         "[AceJump] Internal error: tree node type is invalid")))))
...
Move the same position to the end of `ace-jump-mode-mark-ring'." 
  (lexical-let 
      ((mp (mark t)) 
       (cb (current-buffer))) 
    (if mp 
        (setq ace-jump-mode-mark-ring (ace-jump-move-first-to-end-if ace-jump-mode-mark-ring 
                                                                     (lambda (x) 
                                                                       (and (equal
                                                                             (aj-position-offset x)
                                                                             mp) 
                                                                            (eq (aj-position-buffer
                                                                                 x) cb))))))))
...
Move the aj-position with the same buffer to the end of `ace-jump-mode-mark-ring'." 
  ...
  (let ((index global-mark-ring))
    ...
    (while (and index 
                (not (marker-buffer (car index)))) 
      (setq index (cdr index))) 
    (if index
        ;; find the mark
        (lexical-let 
            ((mb (marker-buffer (car index)))) 
          (setq ace-jump-mode-mark-ring (ace-jump-move-to-end-if ace-jump-mode-mark-ring 
                                                                 (lambda (x) 
