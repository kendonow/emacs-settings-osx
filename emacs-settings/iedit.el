...
(defcustom iedit-occurrence-face 'hl-yellow
  "*Face used for the occurrences' default values."
  :type 'face  
  :group 'iedit)
...
;    (define-key map (kbd "M-n") 'iedit-next-occurrence)
    (define-key map (kbd "<S-tab>") 'iedit-prev-occurrence)
;    (define-key map (kbd "M-p") 'iedit-prev-occurrence)
...
;;; Default key bindings:
;(define-key global-map "\C-xw" 'iedit-mode)
;(global-set-key (kbd "C-x w")    (quote iedit-mode))
