...
;  (define-key cscope-list-entry-keymap "q" 'cscope-quit)
  ...
  (define-key cscope-list-entry-keymap "s" 'cscope-find-this-symbol-no-prompting-no-updates)
...
;  (define-key cscope-list-entry-keymap "d" 'cscope-find-this-symbol)
  ...
  (define-key cscope-list-entry-keymap "g" 'cscope-find-global-definition-no-prompting-no-updates)


;  (define-key cscope-list-entry-keymap "g" 'cscope-find-global-definition-no-promptidebng)
  (define-key cscope-list-entry-keymap "c" 'cscope-find-functions-calling-no-prompting-no-updates)
;  (define-key cscope-list-entry-keymap "C" 'cscope-find-functions-calling-this-function)
;  (define-key cscope-list-entry-keymap "\C-c" 'cscope-find-called-function-no-updates)
  (define-key cscope-list-entry-keymap "d" 'cscope-find-called-functions-no-updates)


  (define-key cscope-list-entry-keymap "t" 'cscope-find-this-text-string-no-updates)
...
;  (define-key cscope-list-entry-keymap "." 'cscope-goto-next-set-value)
 ; (define-key cscope-list-entry-keymap "," 'cscope-goto-prev-set-value)

  ;; ---
  (define-key cscope-list-entry-keymap "r" 'cscope-wash-keep)
  (define-key cscope-list-entry-keymap "R" 'cscope-wash-flush)
  (define-key cscope-list-entry-keymap "U" 'cscope-wash-undo)


;  (define-key cscope-list-entry-keymap "a" 'cscope-set-initial-directory)
  (define-key cscope-list-entry-keymap "a" 'cscope-create-list-of-files-to-index)
...
(if cscope:map
  ...
  (define-key cscope:map "\C-csg" 'cscope-find-global-definition-no-prompting-no-updates)
  (define-key cscope:map "\C-csc" 'cscope-find-functions-calling-this-function-no-updates)
  (define-key cscope:map "\C-csf" 'cscope-find-called-functions-no-updates)
  ...
  (define-key cscope:map "\C-cse" 'cscope-find-egrep-pattern-no-updates)
...
;  (define-key cscope:map "\C-csl" 'cedet-cscope-create/update-database)
  (define-key cscope:map "\C-csl" 'cscope-shell-create-cscope-out)
...
;  (define-key cscope:map "\C-csS" 'cscope-tell-user-about-directory)
 ; (define-key cscope:map "\C-csT" 'cscope-tell-user-about-directory)
  ...
  (define-key cscope:map "\C-csr" 'cscope-wash-keep)
  (define-key cscope:map "\C-csR" 'cscope-wash-flush)
  (define-key cscope:map "\C-csU" 'cscope-wash-undo)
...
;            [ "find next set value"     cscope-goto-next-set-value t ]
 ;           [ "find prev set value"     cscope-goto-prev-set-value t ]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 (defun cscope-wash-flush (towash)
   "Flush lines match regexp towash"
   (interactive 
    (list (let (x) (setq x (read-from-minibuffer 
                            "flush lines(Regexp): " nil)))))
   (cscope-wash towash 'flush-lines))
 
 (defun cscope-wash-keep (tokeep)
   "Only keep result lines that match regexp tokeep"
   (interactive 
    (list (let (x) (setq x (read-from-minibuffer 
                            "keep lines(Regexp): " nil)))))
   (setq tokeep (concat tokeep "\\|^\\*\\*\\* .*:$"))
   (cscope-wash tokeep 'keep-lines))
 
 (defun cscope-wash (regexp fun)
   (save-excursion
 ...
 (defun cscope-wash-undo ()
   (interactive)
   (let (buffer-read-only)
     (undo)))


;; (defun cscope-goto-next-set-value ()
;;   (interactive)
;;   (search-forward-regexp "[a-zA-Z0-9_]+\\(.\\|->\\)[a-zA-Z0-9_]+[ \\t]*=[ \\t]*[a-zA-Z0-9_]+[ \\t]*;")
;;   (backward-word))
;; 
;; (defun cscope-goto-prev-set-value ()
;;   (interactive)
;;   (search-backward-regexp "[a-zA-Z0-9_]+\\(.\\|->\\)[a-zA-Z0-9_]+[ \\t]*=[ \\t]*[a-zA-Z0-9_]+[ \\t]*;")
;;   (backward-word))
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defun cscope-shell-create-cscope-out ()
  (interactive)
  (shell-command "cscope -b -q -k")
  (message "cscope-shell-create-cscope-out")
)
...
;      (setq cscope-do-not-update-database nil)
      (cscope-shell-create-cscope-out)
;    (cscope-search-one-database)
))

;(defadvice cscope-create-list-of-files-to-index (after slickcopy activate compile)
