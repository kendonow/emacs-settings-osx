...
(defun cscope-find-egrep-pattern-no-updates (symbol)
  "Display egrep pattern."
  (interactive (list (cscope-prompt-for-symbol "Find egrep pattern  [without updating the database]: " nil)))
  (let (
        (cscope-do-not-update-database t)
	(cscope-adjust t))
    (setq cscope-symbol symbol)
    (cscope-call (format "find egrep pattern: %s" symbol)
		 (list "-6" symbol) nil 'cscope-process-filter
		 'cscope-process-sentinel)
 ; (setq cscope-do-not-update-database t)
))

(defun cscope-find-called-functions-no-updates (symbol)
  "Display functions called a function."
  (interactive (list (cscope-prompt-for-symbol "Find functions called this function [without updating the database]: " nil)))
  (let (
        (cscope-do-not-update-database t)
	(cscope-adjust t))
    (setq cscope-symbol symbol)
