...
(if cscope-list-entry-keymap
    nil
  ;; (define-prefix-command 'cscope-list-entry-keymap)
...
(defun cscope-search-one-database ()
  ...
  (let ( next-item options cscope-directory database-file outbuf done
    ...
    ;; add by feigo.zj
    (insert (format "cscope-do-not-update-database = %s" cscope-do-not-update-database))
    ;; (insert "\nDatabase update: %s " cscope-do-not-update-database)
...
(defun cscope-unix-index-files-sentinel (process event)
  ...
  (let (buffer)
    ...
    (insert (format "cscope-do-update-database = %s" (not cscope-do-not-update-database)))
	(delete-process process)
...
(defun cscope-find-egrep-pattern (symbol)
  ...
  (let ( (cscope-adjust t)
         ( cscope-do-not-update-database nil) )	 ;; Use fuzzy matching.
    (setq cscope-symbol symbol)
...
;; (add-hook 'fundamental-mode (function cscope:hook))
(add-hook 'rc-generic-mode-hook (function cscope:hook))

;; (if cscope:map
;;   (define-key-lexical-1  "\C-csn" '( (?n   cscope-next-symbol ) 
