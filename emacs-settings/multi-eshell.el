...
(defun multi-eshell-function () "This function opens the appropriate shell." 
  (let(
      ...
      (let ( (tempname (generate-new-buffer-name "*tempshell*"))
        ...
        (if (eq (get-buffer multi-eshell-name) nil) ;If a
            (progn
              ...
              (setq multi-eshell-index (+ multi-eshell-index 1))
              ;; (setq tempname (concat new-buff-name "-"  localdir ))
        ...
        ;; (setq localdir (concat "~/" (file-relative-name localdir (getenv "HOME"))))
        ;; (setq tempname (concat new-buff-name "-"  localdir ))
        ;; (rename-buffer tempname)
