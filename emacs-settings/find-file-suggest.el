...
(defun find-file-suggest (&optional revert-last-query)
  (interactive)
  ...
  (if (ffs-get-current-file-index)
      (progn
        ...
        (read-string (format "Find indexed file (%s): " ffs-current-file-index-name) (if revert-last-query ffs-last-query))
...
(defun ffs-grep (&optional revert-last-query)
  (interactive)
  (if (not (ffs-get-current-file-index))
      (message "Warning: there is no current file index in use (you can set one with M-x ffs-use-file-index, after having created it with M-x ffs-create-file-index)"))
  (if (ffs-get-current-file-index)
      (progn
        ...
        (add-hook 'minibuffer-exit-hook 'ffs-minibuffer-exit) 
        (read-string (format "FFS-grep (%s): " ffs-current-file-index-name) (if revert-last-query ffs-grep-last-query))
        ;; this is executed after the minibuffer is exited (user hit <ret> or clicked a file)
        (if (not (or (string= ffs-picked-file "") (string= ffs-picked-file "<no match>")))
            (find-file ffs-picked-file)))))
...
(defun ffs-create-file-index (name dir include-filter exclude-filter)
  ...
  (if (string-match (downcase (file-truename dir)) (downcase (file-truename default-directory)))
      (progn 
...
(defun ffs-revert-last-query ()
  (interactive)
  (find-file-suggest t))

(defun ffs-grep-revert-last-query ()
  (interactive)
  (ffs-grep t))
