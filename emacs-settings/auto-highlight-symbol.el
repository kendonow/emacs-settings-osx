...
(if auto-highlight-symbol-mode-map
  ...
  (setq auto-highlight-symbol-mode-map
        (let ((map (make-sparse-keymap)))
          ...
          (define-key map (kbd "M-a"    ) 'ahs-backward            )
          (define-key map (kbd "M-e"   ) 'ahs-forward             )
          (define-key map (kbd "M-<right>"   ) 'ahs-forward             )
          (define-key map (kbd "<tab>"   ) 'ahs-forward             )
          (define-key map (kbd "<backtab>"   ) 'ahs-backward             )
          ...
          (define-key map (kbd "C-'"     ) 'ahs-change-range        )
          (define-key map (kbd "C-;"     ) 'ahs-edit-mode           )
          (define-key map (kbd "C-g"     ) 'toggle-off-auto-high-mode   )
          map)))
...
(defun toggle-off-auto-high-mode ( )
  (interactive)
...
(defun toggle-global-auto-high-mode ( )
  (interactive)
   (if auto-highlight-symbol-mode
      ( auto-highlight-symbol-mode -1) 
    ( auto-highlight-symbol-mode 1) 
