...
(defun smart-whole-line(bcopy  &optional arg) 
  ...
  (let ((argtemp arg) 
    ...
    ;; (setq end (line-end-position arg)))
    (when (> end (point-max)) 
      (message  "is buffer end") 
      (setq end (- end 1)))
...
(defun smart-class-name(bcopy &optional arg) 
  (if smart-kill-negative-arg 
      (setq arg (- 0 arg))) 
  (save-excursion 
    (test-point-forward 'symbol) 
...
(defun smart-class-name-kill( ) 
  (interactive ) 
  (toggle-off-smart-kill-mode) 
  (smart-class-name smart-kill-flag 1))
...
(if smart-kill-keymap 
  ...
  (define-key smart-kill-keymap "b"    'copy-buffer-filename-as-kill     ) 
  (define-key smart-kill-keymap "B"    'copy-buffer-directory-as-kill     ) 
  (define-key smart-kill-keymap "\C-b"    'copy-buffer-fullfilename-as-kill     ) 
  ...
  (define-key smart-kill-keymap "c"    'smart-class-name-kill             ) 
...
(defun smart-kill-ring-save (&optional arg) 
  ...
  (if mark-active 
    ...
    (progn 
      ...
      (define-key smart-kill-keymap "\C-w" 'close-smart-kill-mode               ) 
      (define-key smart-kill-keymap "\C-s" 'close-smart-kill-mode             ) 
      (define-key smart-kill-keymap "\C-q" 'close-smart-kill-mode) 
      (define-key smart-kill-keymap "\C-l" 'close-smart-kill-mode)
      ( smart-kill-mode 1))))
...
(defun smart-kill-region (&optional arg) 
  ...
  (if mark-active 
    ...
    (progn 
      ...
      (setq smart-kill-arg arg) 
      (when 
          (featurep 'smart-yank-mode) 
        (define-key smart-kill-keymap "y" 
          (lambda() 
            (interactive) 
            ( smart-kill-mode -1) 
            (smart-yank-call)))
        (define-key smart-kill-keymap "\C-s" 
          (lambda() 
            (interactive ) 
            ( smart-kill-mode -1) 
            (smart-yank-symbol 1))) 
        (define-key smart-kill-keymap "\C-w" 
          (lambda() 
            (interactive )
            ( smart-kill-mode -1) 
            (smart-yank-word 1)))
        (define-key smart-kill-keymap "\C-q" 'smart-yank-quote-string) 
        (define-key smart-kill-keymap "\C-l" 'smart-yank-list) 
        (smart-kill-mode 1)))))
